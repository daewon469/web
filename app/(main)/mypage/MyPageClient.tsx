...
export default function MyPageClient() {
  ...
  const fetchList = useCallback(
    async (reset: boolean, currentCursor?: string) => {
      ...
      setError(null);
      try {
        const { items: list, next_cursor } = await Posts.mylist(1, me, {
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-xl font-bold text-[#0B1B3A]">내 구인글</h1>
        ...
        return (
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              ...
              />
              <ActionBtn label="수정" onClick={() => router.push(`/write?id=${item.id}`)} />
              <ActionBtn
                label="마감"
